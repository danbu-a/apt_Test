...
// 알고리즘입니다(온디맨드 화면 반영 로직과 이 서버 영구반영 로직 양쪽이 같은 판단
// 기준을 써야 해서 그대로 옮겨왔습니다 - 알고리즘을 바꿀 땐 두 곳 모두 반영해야 합니다).
...
// turnover_results.json에 대한 읽기-수정-쓰기를 프로세스 내에서 직렬화하는 큐입니다.
// 서로 다른 aptKey 요청이 거의 동시에 들어와도, 둘 다 파일을 읽었다가 서로의 쓰기를
// 덮어써버리는 lost-update를 막으려면 "읽기+쓰기 전체"가 한 번에 하나씩만 실행돼야
// 합니다. 이 프로미스 체인이 그 역할을 합니다 - 외부 락 없이, 이전 작업이 끝나야
// (성공/실패 무관) 다음 작업의 readFile이 시작됩니다.
let persistQueueTail = Promise.resolve();
...
function persistRegistryMatchToResults(aptKey, result) {
  // 이번 호출의 실행을 큐 끝에 매달아, 앞선 호출의 읽기+쓰기가 완전히 끝난 뒤에만
  // 시작되게 합니다.
  const run = persistQueueTail.then(() => persistRegistryMatchToResultsLocked(aptKey, result));
  // 큐 자체는 항상 "성공"으로 이어가야 뒤에 대기 중인 다른 aptKey 작업이 계속 진행됩니다 -
  // 이번 호출의 실패(에러)는 run이 반환하는 프로미스로만 호출부에 전달됩니다.
  persistQueueTail = run.catch(() => {});
  return run;
}
...
async function persistRegistryMatchToResultsLocked(aptKey, result) {
  ...
  // (dashboard.js refineUnitCountsFromRegistry와 동일한 이유). 서버가 표제부 동번호
  // 구간으로 이 단지 몫만 분리하지 못했는데(dongFiltered=false) 같은 지번을 여러 단지가
  // 쓰는 경우는 result.areas가 섞인 값일 수 있어 반영을 건너뜁니다.
  ...
  // K-apt/부동산원 공식 총세대수는 파일에서 직접 다시 계산합니다(클라이언트가 보낸
  ...
  try {
    // 임시 파일에 먼저 다 쓴 뒤 rename으로 교체합니다 - 쓰는 도중 서버가 죽거나
  ...
  const server = http.createServer(async (req, res) => {
    ...
    if (requestUrl.pathname === "/api/unit-types") {
      ...
      try {
        ...
        // 응답은 먼저 보내고, turnover_results.json 반영은 백그라운드로 진행합니다(사용자
        // 화면 응답 속도에 영향 주지 않기 위함). 실패해도 온디맨드 조회 자체(위 응답)는
        // 이미 성공했으므로 사용자에게 별도 에러가 노출되지 않습니다 - 다음에 이 단지를
        // 다시 열면 재시도됩니다.
        if (result && result.status === "ok") {
          persistRegistryMatchToResults(aptKey, result).catch(err => {
            console.error(`[등기 실측 영구반영 ERROR] ${aptKey}: ${err.message}`);
          });
        }
      } catch (err) {
